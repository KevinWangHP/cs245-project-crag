...
if __name__ == '__main__':
    ...
    def trim_html_tree_rerank(context_window, dataset):
        data_file = f"./html_data/{dataset}/tree-rerank/{max_node_words}-{dataset}-{split}.jsonl"
        data_lines = [json.loads(line) for line in open(data_file)]
        loguru.logger.info(f"read {len(data_lines)} node lines from {data_file}")
        output_file = f"./html_data/{dataset}/tree-rerank/{chat_tokenizer_name}/{max_node_words}-{dataset}-{split}-{context_window}.jsonl"
