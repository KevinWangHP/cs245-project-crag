...
class Retriever2:
    def __init__(self, device1,device2,batch_size=64,  
                 ...
                 separators=['.', "\n", " ", ""], token_path="../pretrained_model/meta-llama/Llama-3.2-3B-Instruct",
                stopwords_list = 'db3/processed_data/stopwords_list.npy',rerank_path ='BAAI/bge-reranker-v2-m3'):
        ...
        self.oscar_map = np.load('db3/processed_data/oscar_map.npy',allow_pickle=True).tolist()
        self.oscar_map_dlc = np.load('db3/processed_data/oscar_map_dlc.npy',allow_pickle=True).tolist()
        self.imdb_movie_dataset = np.load('db3/processed_data/all_imdb_movie.npy',allow_pickle=True).tolist()
        self.movie_index={}
        ...
        self.grammy_map = np.load('db3/processed_data/grammy.npy',allow_pickle=True).tolist()
        self.ticker_name_map,self.ticker_info_map,self.ticker_name_set_map = np.load('db3/processed_data/finance_data.npy',allow_pickle=True).tolist()
    ...
    def init_retriever(self, search_results, recall_k=50, task3_topk = 5,max_length= 20000, task3 = False, separator=' ', method='ensemble', query=None, riddle=100,time_half_limit=1):
        st = time()
        self.method = method
        docs = []
        hashes = set()
        ...
        for idx, html in enumerate(search_results[:task3_topk]):
            ...
            inputs = self.tokenizer.encode(text,max_length=max_length,add_special_tokens=False)
            print('HTML text length: ',len(inputs))
            if len(inputs)==max_length:
                text = self.tokenizer.decode(inputs)
